...
def basic_experiment(x_train, y_train, x_test, y_test, formatted_print=False):
    ...
    y_bin = np.where(y_test == 'B', 0, 1)
    pred_bin = np.where(predictions == 'B', 0, 1)
    acc = accuracy(y_bin, pred_bin)
    # ========================
...
# def M_experiment(x_train, y_train, x_test, y_test, M_list):
#     """
#     Use ID3 model, to train on the training dataset and evaluating the accuracy in the test set.
#     """
...
#         x_t = np.copy(x_train)
#         y_t = np.copy(y_train)
#         id3.pruned_fit(x_t, y_t,m)
#         x_test_copy = np.copy(x_test)
#         predictions = id3.predict(x_test_copy)
#         pred_bin = np.where(predictions == 'B', 0, 1)
#         y_bin_copy = np.copy(y_bin); 
#         m_acc.append(accuracy(y_bin_copy, pred_bin))
#     # ========================
#     # id3 = ID3(['B','M'])
#     # id3.pruned_fit(x_train, y_train,M_list)
#     # predictions = id3.predict(x_test)
#     # y_bin = np.where(y_test == 'B', 0, 1)
#     # pred_bin = np.where(predictions == 'B', 0, 1)
#     # print(accuracy(y_bin, pred_bin))
#     # ========================
#     for m,acc in zip(M_list, m_acc):
#         print(f"For M = {m} we get the resulting accuracy: {acc}")
#     plt.plot(M_list, m_acc)
#     plt.xlabel('M')
#     plt.ylabel('accuracy')
#     for x,y in zip(M_list, m_acc):
#         round_y = float("{:.3f}".format(y)) * 100
#         plt.annotate(f"({x},{round_y}%)", xy =(x,y), fontsize=8)
#     plt.show()
...
def best_m_test(x_train, y_train, x_test, y_test, min_for_pruning):
    ...
    # ====== YOUR CODE: ======
    id3 = ID3(['B','M'])
    id3.pruned_fit(x_train, y_train,min_for_pruning)
    predictions = id3.predict(x_test)
    y_bin = np.where(y_test == 'B', 0, 1)
    pred_bin = np.where(predictions == 'B', 0, 1)
    acc = accuracy(y_bin, pred_bin)
...
if __name__ == '__main__':
    ...
    # exit(0)
    # M = [1,2,5,10,20,25,30,35,40,50]
    # # M = [1]
    # M_experiment(*data_split, M)
    # exit(0)
